...
@onready var _world_size = sprite.get_rect().size
...
func _world_to_viewport(point : Vector2) -> Vector2:
	...
	if sprite.centered == true:
		parent_offset = Vector2(sprite.position.x  - _world_size.x/2, sprite.position.y  - _world_size.y/2) 
	return Vector2(
